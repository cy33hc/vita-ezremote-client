...
{
    return std::string("http://localhost:" + std::to_string(gg_app.gg_auth_server_port) + "/google_auth");
}
...
{
    Client auth_client(std::string("http://")+gg_app.gg_auth_server+":"+std::to_string(gg_app.gg_auth_server_port));
    auth_client.enable_server_certificate_verification(false);
    auth_client.set_follow_location(true);
    std::string url = std::string("/auth");

    std::string gg_auth_url = std::string(GOOGLE_AUTH_URL "?client_id=") +
                           BaseClient::EncodeUrl(gg_app.client_id) +
                           "&redirect_uri=" + BaseClient::EncodeUrl(GetRedirectUrl()) +
                           "&response_type=code&access_type=offline&scope=" + BaseClient::EncodeUrl(GetScopes()) +
                           "&include_granted_scopes=true";
    std::string auth_url=std::string("/auth?client_id=") + BaseClient::EncodeUrl(gg_app.client_id) +
                         "&client_secret=" + BaseClient::EncodeUrl(gg_app.client_secret) +
                         "&url=" + BaseClient::EncodeUrl(gg_auth_url);

    if (auto res = auth_client.Get(auth_url))
    {
        if (!HTTP_SUCCESS(res->status))
        {
    ...
    SceRtcTick curr_tick;
    sceRtcGetCurrentTick(&curr_tick);
    uint64_t expires_in = curr_tick.tick + 300000000;

    login_state = 0;
    while (curr_tick.tick < expires_in)
    {
        std::string url = std::string("/get_auth");
        if (auto res = auth_client.Get(url))
        {
            if (HTTP_SUCCESS(res->status))
            {
                json_object *jobj = json_tokener_parse(res->body.c_str());
            ...
            if (HTTP_SUCCESS(res->status))
            {
                ...
                if (json_object_get_type(files) == json_type_array)
                {
                    ...
                    for (size_t idx = 0; idx < afiles->length; ++idx)
                    {
                        ...
                        Util::convertUtcToLocalTime(&utc, &local);
...
void *GDriveClient::RefreshTokenThread(void *argp)
{
    ...
    {
        SceRtcTick tick;
        memset(&tick, 0, sizeof(SceRtcTick));
        sceRtcGetCurrentTick(&tick);
